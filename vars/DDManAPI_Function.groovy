...
def git_own_f(String WorkSpace1,String Branchname,String Repository){
    ...
    def proc
    def cmd = "git init".execute() | "git fetch --progress ${Repository} +refs/heads/${Branchname}:refs/remotes/${Branchname} --depth 1".execute() | "git --git-dir=.\\\\.git --work-tree=.\\\\. checkout ${Branchname} -f".execute() | "rd .git /S /Q".execute()
    proc=cmd.execute()
